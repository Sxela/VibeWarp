"""IP-Adapter UNet hooks (PlugableIPAdapter).

Vendored from Sxela/sxela-stablediffusion `modules/controlmodel_ipadapter.py`
(the fork the reference WarpFusion install uses editable). Ships with the two
`.data` files holding unconditional CLIP-vision embeddings for plus models.

Patches vs upstream:
- `.data` files load with map_location='cpu' (upstream loads to cuda at import
  time; the tensors are `.to(cond)`-moved at every use site, so this is
  equivalent and avoids CUDA init on import).

The lazy `from ldm/sgm.modules.attention import CrossAttention` imports inside
set_model_attn2_replace / set_model_patch_replace intentionally stay bare:
they must resolve to the SAME module the loaded UNet was built from
(vibewarp/vendor is on sys.path via model_loader._ensure_vendor_on_path).
"""
...
clip_vision_h_uc = torch.load(clip_vision_h_uc, map_location='cpu')['uc']

clip_vision_vith_uc = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'clip_vision_vith_uc.data')
clip_vision_vith_uc = torch.load(clip_vision_vith_uc, map_location='cpu')['uc']
