...
class FrozenCLIPEmbedder(AbstractEmbModel):
    ...
    def forward(self, text):
        ...
        if self.layer == "last":
            ...
            # transformers 5.x no longer returns CLIP intermediate states even
            # when output_hidden_states is requested (same issue the ldm copy
            # works around). Reproduce the selected hidden state by stopping the
            # text encoder early.
            #
            # hidden_states[i] is the INPUT to layer i, so it equals running the
            # first i layers. SDXL's config uses a positive layer_idx (11), while
            # SD1.x clip-skip uses a negative one (-2) — handle both.
            from transformers.masking_utils import create_causal_mask

            text_model = self.transformer.text_model
            layers = text_model.encoder.layers
            layers_to_run = (
                self.layer_idx if self.layer_idx >= 0
                else len(layers) + self.layer_idx + 1
            )
            z = text_model.embeddings(input_ids=tokens, position_ids=None)
            attention_mask = create_causal_mask(
                config=text_model.config, inputs_embeds=z,
                attention_mask=None, past_key_values=None,
            )
            for encoder_layer in layers[:layers_to_run]:
                z = encoder_layer(z, attention_mask, is_causal=True)

        if self.layer == "hidden" and getattr(self, 'apply_final_layer_norm', True):
            # A1111/WarpFusion clip-skip applies CLIP's final layer norm to the
            # selected earlier transformer state — the notebook loads sd_hijack,
            # which does exactly this, and the ldm copy of this encoder does too.
            # Without it the crossattn conditioning comes out ~10x too large
            # (raw hidden-state std ~4.9 vs ~0.4 normalised), which overweights the
            # prompt: renders track the prompt harder but diverge from the notebook.
            # Off = that stronger-prompt behaviour, kept as an opt-out
            # (DiffusionConfig.clip_final_layer_norm).
...
class FrozenOpenCLIPEmbedder2(AbstractEmbModel):
    ...
    def encode_with_transformer(self, text):
        ...
        batch_first = bool(getattr(first_block.attn, "batch_first", False))
        if not batch_first:
    ...
    def text_transformer_forward(self, x: torch.Tensor, attn_mask=None):
        ...
        def as_nld(value):
            return value if batch_first else value.permute(1, 0, 2)

        for i, r in enumerate(self.model.transformer.resblocks):
            if i == len(self.model.transformer.resblocks) - 1:
                outputs["penultimate"] = as_nld(x)
            if (
...
class FrozenOpenCLIPImageEmbedder(AbstractEmbModel):
    ...
    def preprocess(self, x):
        import kornia  # vendored patch: lazy — only this embedder needs it
        # normalize to [0,1]
