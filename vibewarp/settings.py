...
def load_warpfusion_settings(settings_path: str, models_root: Optional[str] = None) -> Dict[str, Any]:
    ...
    if model_path:
        inferred = os.path.dirname(os.path.dirname(os.path.abspath(model_path)))
        if inferred:
            model_roots.append(inferred)

    if model_path and not os.path.isfile(model_path):
        resolved = _find_in_roots(os.path.basename(model_path), model_roots,
                                  ('checkpoints', ''))
        if resolved:
            model_path = resolved
    ...
    config = {
        ...
        'flow': {
            ...
            'missed_consistency_dilation': raw.get('missed_consistency_dilation', 1),
